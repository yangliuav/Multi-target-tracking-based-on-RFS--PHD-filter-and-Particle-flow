    ...
    if exist(Q_name,'file') 
        ...
        for i=1:size(vgset,2)
            ...
            if  dist_line(1,i)>0
                ...
                temp_P1(1,1)        =   vgset(1,i) + dist_mov(i)*cosd(delta1)*BD_cof;     % Spread particles x position
        ...
        %             X(3,i)  =   X(3,i) + dist_mov(i)*sind(delta2)*BD_cof;  % Spread particles y position  
                    vgset(1,i)  =   vgset(1,i) + 0.5*dist_mov(i)*cosd(delta2)*BD_cof;   % Spread particles x position
                    if 235-vgset(1,i)>5
