function tracking_output = run_one_trial(ps,trial_ix,video)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ismember('NPF-SMC_PHD',ps.algs)
    ps_new = ps;
    ps_new.pf_type = 'NPF';
    tracking_output.NPFSMC = SMCPHD(ps_new,y,video);
end
if ismember('NPF-SMC_PHD_S',ps.algs)
    ps_new = ps;
    ps_new.pf_type = 'NPFS';
    tracking_output.NPFSMCS = SMCPHD(ps_new,y,video);
end
...
if ismember('SMC_PHD',ps.algs)
    ...
    addpath( Init_PATH ); 
    tracking_output.SMC_PHD = SMCPHD(ps_new,y,video);
end

if ismember('ZPF-SMC_PHD',ps.algs)
    ps_new = ps;
    ps_new.pf_type = 'ZPF';
    tracking_output.ZPFSMC = SMCPHD(ps_new,y,video);
end
