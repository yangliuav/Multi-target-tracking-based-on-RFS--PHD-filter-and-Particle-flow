function Sensor = ipf_smc_phd_v1(t,setup,iy,Sensor)
...
%%
clutter = 0.0125;%0.0035;%
setup.Example.propparams.time_step = t;

%% step 1&2: Propagate survival particles and calculate w_{k|k-1}
[Sensor.vg,Sensor.vgset,setup] = propagateAndEstimatePriorCovariancePHD(Sensor,setup);

%% step 3&4: Birth model generate new born particles based on measurement
% and update new born particles weights
[Sensor.vgset,surv_particle_num] = birth_model(setup,iy(:,t),clutter,Sensor);

%% step 5: IPF on survival particles
[Sensor.vgset,setup] = pflow_update(Sensor,setup,iy(:,t),clutter,surv_particle_num,Cz,nSi);

%% step 6: Compare new born particle and survive particle to modify weights
Sensor.vgset = compare_particle(Sensor,surv_particle_num);

%% step 7: Calculate the estimated number of targets
vgset = Sensor.vgset;        
sumw = 0;
sumw2 = 0;
...
for i = 1:size(vgset,2)
    ...
    if size(vgset(i).wlik,2) == 1
        ...
        j = floor((i-surv_particle_num)/setup.nParticle)+1;
...
inx = particle_weight>0.002;%0.005;%
